...
const Layout = () => {
  ...
  // Prefetch map: route segment -> dynamic import
  const prefetchMap: Record<string, () => Promise<unknown>> = {
    'workflows': () => import('./WorkflowsPage'),
    'agents': () => import('./EnhancedAgentsPage'),
    'reports': () => import('./ReportsPage'),
    'statistics': () => import('./StatisticsPage'),
    'marketplace': () => import('./MarketplacePage'),
    'farmer-profiles': () => import('./FarmerProfilePage'),
    'business-intelligence': () => import('./BusinessIntelligencePage'),
    'demo': () => import('../pages/DemoPage'),
  };

  const handlePrefetch = (path: string) => {
    if (prefetchMap[path]) {
      prefetchMap[path]().catch(console.error);
    }
  };

  // NOTE: Use relative paths to match router configuration
    ...
    <div className="dashboard">
      ...
      <nav className="sidebar" aria-label="Primary">
        <div className="logo-container" role="banner">
          <NavLink to="/" className="logo-link" end aria-label="AgriMitr Home">
            <div className="sidebar-logo brand-logo" aria-hidden="true">
              <img
                src="/logo.png"
                alt="AgriMitr Logo"
                ...
                loading="eager"
                onError={(e) => {
                  // graceful fallback to vector if image missing
                  const fallback = `<svg width='52' height='52' viewBox='0 0 64 64' fill='none' xmlns='http://www.w3.org/2000/svg'><circle cx='32' cy='32' r='30' fill='rgba(255,214,10,0.08)' stroke='#FFD60A' stroke-width='2'/><path d='M32 10C24 18 20 26 20 34C20 44 26 52 32 54C38 52 44 44 44 34C44 26 40 18 32 10Z' fill='#FFD60A' stroke='#1E293B' stroke-width='1.5' stroke-linejoin='round'/><path d='M32 18V46' stroke='#1E293B' stroke-width='2' stroke-linecap='round'/><path d='M26 28H32M32 38H38' stroke='#1E293B' stroke-width='2' stroke-linecap='round'/></svg>`;
                  (e.target as HTMLImageElement).outerHTML = fallback; // swap element
                }}
              />
            </div>
            <div className="logo-text brand-text" aria-label="AgriMitr brand">
              <h1 className="brand-title">AgriMitr</h1>
              <p className="brand-tagline">Smart Agriculture Platform</p>
            </div>
          </NavLink>
        </div>
        ...
        <div className="nav-items" role="menu">
          {navItems.map(item => (
            ...
            >
              ...
              <span className="nav-loader"></span>
            </NavLink>
          ))}
        </div>
      ...
      <div className={`main-content ${isNavigating ? 'navigating' : ''}`} role="main">
        <Outlet />
      </div>
